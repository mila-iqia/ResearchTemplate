...
def test_first_batch(
    ...
    for gitignore_file in [
        parent / ".gitignore" for parent in reversed(figure_path.relative_to(REPO_ROOTDIR).parents)
    ]:
        if not gitignore_file.exists():
            continue
        if any(line.strip() == "*.png" for line in gitignore_file.read_text().splitlines()):
            return
        break
    logger.info(f"Adding lines to {gitignore_file}")
